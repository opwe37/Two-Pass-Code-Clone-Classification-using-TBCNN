...
def init_binary_net(feature_size):
    ...
    with tf.name_scope("network"):
        conv1 = conv_layer(1, 50, nodes, children, feature_size)
        pooling = pooling_layer(conv1)

    return nodes, children, pooling


def init_multi_net(feature_size):
    """Initialize an empty network."""
    with tf.name_scope("inputs"):
        nodes = tf.placeholder(tf.float32, shape=(None, None, feature_size), name='tree')
        children = tf.placeholder(tf.int32, shape=(None, None, None), name='children')
    ...
    with tf.name_scope("network"):
        # conv1 = conv_layer(4, 50, nodes, children, feature_size)
        conv1 = conv_layer(1, 50, nodes, children, feature_size)
        pooling = pooling_layer(conv1)
...
def conv_layer(num_conv, output_size, nodes, children, feature_size):
    """Creates a convolution layer with num_conv convolutions merged together at the output.
    Final output will be a tensor with shape[batch_size, num_nodes, output_size * num_conv]"""
...
def conv_step(nodes, children, feature_size, w_t, w_r, w_l, b_conv):
    ...
    with tf.name_scope('conv_step'):
        ...
        with tf.name_scope('combine'):
            ...
            # output is (batch_size, max_tree_size, output_size)
            return tf.nn.relu(result + b_conv, name='conv')


def pooling_layer(nodes):
    """Cereates a max dynamic pooling layer from the nodes."""
...
def hidden_layer(pooled, input_size, output_size):
    ...
    with tf.name_scope("hidden"):
        ...
        return lrelu(tf.matmul(pooled, weights) + biases, 0.1)
        # return tf.nn.relu(tf.matmul(pooled, weights))
